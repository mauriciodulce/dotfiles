...
z() {
    ...
    if [ -n "$1" ]; then
        if [ -d ~/Code/Dulce/$1 ]; then
            cd ~/Code/Dulce/$1
            return
        # Elimina las siguientes líneas relacionadas con Statamic
    ...
    doubleThreeProjects=($(basename -a $(ls -d ~/Code/Dulce/*/)))
    ...
    cd $(find ~/Code/Dulce/ -type d -name "$selected_dir" -print -quit)
}
